...
#
# Usage:
#   OPENAI_API_KEY=sk-... scripts/local-test.sh   # full run with defaults (OpenAI)
...
NS="${NS:-kscribe-system}"          # operator / release namespace
TEST_NS="${TEST_NS:-kscribe-test}"  # where failing pods (and their diagnoses) live
RELEASE="${RELEASE:-kscribe}"
IMAGE="${IMAGE:-ghcr.io/amjadjibon/kscribe:latest}"
# LLM target (default OpenAI). The base URL must include the API version segment
...
[ -n "$LLM_API_KEY" ] || die "no LLM API key — export OPENAI_API_KEY or pass LLM_API_KEY=..."
...
generate_incidents() {
  log "Creating failing pods in $TEST_NS (BackOff / Failed / FailedScheduling)"
  kubectl create namespace "$TEST_NS" --dry-run=client -o yaml | kubectl apply -f - >/dev/null
  kubectl -n "$TEST_NS" delete pod "${TEST_PODS[@]}" --ignore-not-found >/dev/null 2>&1 || true
  kubectl -n "$TEST_NS" run crasher  --image=busybox --restart=Always -- /bin/false >/dev/null
  kubectl -n "$TEST_NS" run badimage --image=nope/doesnotexist:latest --restart=Always >/dev/null
  kubectl -n "$TEST_NS" run toobig --restart=Never --image=busybox \
...
cleanup() {
  ...
  kubectl delete namespace "$TEST_NS" --ignore-not-found >/dev/null 2>&1 || true
  if [ -n "${UNINSTALL:-}" ]; then
...
# KSCRIBE_OPERATOR_NAMESPACE="" makes the operator watch cluster-wide so it sees
# events in $TEST_NS; diagnoses then land in the event's namespace ($TEST_NS).
kubectl -n "$NS" set env deploy/"$RELEASE" \
  KSCRIBE_OPERATOR_NAMESPACE="" \
...
generate_incidents
kubectl -n "$TEST_NS" delete ksd --all >/dev/null 2>&1 || true

log "Waiting up to ${TIMEOUT}s for diagnoses (provider=$LLM_PROVIDER model=$LLM_MODEL)"
deadline=$(( $(date +%s) + TIMEOUT ))
while :; do
  phases="$(kubectl -n "$TEST_NS" get ksd -o jsonpath='{range .items[*]}{.status.phase}{"\n"}{end}' 2>/dev/null || true)"
...
log "Results"
kubectl -n "$TEST_NS" get ksd

# Show one full RCA if any incident reached Done.
done_name="$(kubectl -n "$TEST_NS" get ksd -o jsonpath='{range .items[?(@.status.phase=="Done")]}{.metadata.name}{"\n"}{end}' 2>/dev/null | head -1)"
if [ -n "$done_name" ]; then
  log "Sample RCA ($done_name)"
  kubectl -n "$TEST_NS" get ksd "$done_name" -o jsonpath='{.status}' \
