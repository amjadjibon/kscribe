...
ON CONFLICT(namespace, name) DO UPDATE SET
    ...
    involved_object_name      = COALESCE(NULLIF(excluded.involved_object_name, ''), incidents.involved_object_name),
    involved_object_namespace = COALESCE(NULLIF(excluded.involved_object_namespace, ''), incidents.involved_object_namespace),
    reason                    = COALESCE(NULLIF(excluded.reason, ''), incidents.reason),
    message                   = COALESCE(NULLIF(excluded.message, ''), incidents.message),
    phase                     = excluded.phase,
    started_at                = excluded.started_at,
    completed_at              = excluded.completed_at,
    llm_provider              = COALESCE(NULLIF(excluded.llm_provider, ''), incidents.llm_provider),
    llm_model                 = COALESCE(NULLIF(excluded.llm_model, ''), incidents.llm_model),
