...
- **REQ-005**: Streaming `CompleteStream` must emit content deltas via `onDelta` and return the accumulated full `Response` (chat uses content only — no tool-call streaming needed).
...
- [ ] TASK-002: Reimplement `Complete(ctx, req Request) (Response, error)` using `client.Chat.Completions.New(ctx, params)`: map `req.Messages` → `[]openai.ChatCompletionMessageParamUnion` (system/user/assistant/tool, carrying tool_calls on assistant turns and tool_call_id on tool turns), `req.Tools` → the SDK's function-tool params (name/description/parameters), `req.Model` → `openai.ChatModel(c.Model)`. Map the SDK response back to `Response`: `Choices[0].Message.Content`, `.ToolCalls` (→ `agent.ToolCall{ID,Type:"function",Function:{Name,Arguments}}`), `FinishReason`, `Usage.TotalTokens`. On non-2xx / SDK error, return a wrapped error (preserve the "provider error …" style so existing error handling/tests still recognize failures).
...
- [ ] TASK-004: Update `internal/agent/agent_test.go` (and any non-streaming test) so the fake OpenAI server still exercises `Complete` via the SDK: point `NewOpenAIClient` at the `httptest` server URL (SDK will POST `…/chat/completions`), and assert the same behaviors (tool-call round-trip, RCA parsing, provider-error path). Adjust request-path/shape assertions to the SDK's output where they changed.

**Completion criteria**: `go build ./...`, `go vet ./...` pass; `go test ./internal/agent ./internal/controller` pass (the diagnosis tool-call loop works end-to-end through the SDK-backed `Complete`).
...
- TASK-002: Reimplement Complete(ctx, req) using client.Chat.Completions.New(ctx, openai.ChatCompletionNewParams{...}). Map req.Messages -> []openai.ChatCompletionMessageParamUnion: role "system"->openai.SystemMessage, "user"->openai.UserMessage, "assistant"->openai.AssistantMessage (include ToolCalls when present), "tool"->openai.ToolMessage(content, toolCallID). Map req.Tools (ToolDefinition{Function:{Name,Description,Parameters map[string]any}}) -> the SDK's function tool params (params.Tools). Model: openai.ChatModel(c.Model). Map the response back to agent.Response: Choices[0].Message.Content; ToolCalls -> []agent.ToolCall{ID, Type:"function", Function:agent.FunctionCall{Name, Arguments}}; FinishReason (string); Usage.TotalTokens (int). On error, return a wrapped error keeping a "provider error" style message so callers/tests that check for provider failure still work. No encoding/json — sonic only if you must (arguments are already JSON strings; pass through).
...
- TASK-004: Update internal/agent/agent_test.go: the existing fake httptest server(s) should still drive Complete via the SDK — pass srv.URL as the base URL to NewOpenAIClient; the SDK POSTs to <base>/chat/completions, so ensure the fake server responds on that path (adjust the handler path check if it asserted a specific path). Assert the same behaviors: tool-call round-trip (a response with tool_calls is surfaced in Response.Choices[0].Message.ToolCalls), normal content, and the provider-error path (non-2xx -> error). Keep TestDiagnosisAgent_* passing.
...
Tasks:
- TASK-005: Reimplement CompleteStream(ctx, req, onDelta func(string) error) (Response, error) using client.Chat.Completions.NewStreaming(ctx, params) with the same param mapping as Phase 1's Complete (reuse a shared param-builder helper). Iterate: for stream.Next() { chunk := stream.Current(); delta := chunk.Choices[0].Delta.Content; if delta != "" { if err := onDelta(delta); err != nil { return ..., err }; accumulate } }; after the loop check stream.Err(). Return a Response whose Choices[0].Message.Content is the accumulated text (and usage if the SDK exposes it on the final chunk). No encoding/json.
